// FIX: Replaced corrupted file content with valid application code.
...
// FIX: Corrected a typo in the Supabase anon key.
...
const dbSelectColumns = `id, ${Object.values(displayToDbMap).join(', ')}, resumo_sim_nao, atualizador_1_email, inicio_da_linha_de_base`;
...
const App = {
    ...
    async updateAvanco(rowId, newValue) {
        ...
        this.filterAndRender(); // Re-filtra e re-renderiza para aplicar a lógica de status
    ...
    applyStatusLogic() {
        ...
        today.setHours(0, 0, 0, 0); // Normaliza a data para comparar apenas o dia
        ...
        if (pageData.length === 0) {
            ...
            pageData.forEach((row, index) => {
                ...
                displayHeaders.forEach(header => {
                    ...
                    // A cor do texto não se aplica mais à célula de status, que tem seu próprio estilo
                    ...
                    if (header === 'AVANÇO') {
                        this.renderAvancoCell(td, row, isResumoSim);
                    } else if (header === 'STATUS') {
                        this.renderStatusCell(td, row[dbKey]);
                    } else {
                        td.textContent = row[dbKey] || '-';
                    }
    ...
    renderStatusCell(td, status) {
        ...
        td.innerHTML = ''; // Limpa a célula
        const badge = document.createElement('span');
        badge.className = `px-2.5 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${colorClass}`;
