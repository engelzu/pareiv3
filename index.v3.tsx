...
const App = {
    ...
    init() {
        ...
        if (!(window as any).supabase) {
            ...
            this.showError("Erro crítico: A biblioteca de dados não pôde ser carregada.");
            return;
        }
    ...
    cacheElements() {
        const ids = ['loading', 'dataContainer', 'errorContainer', 'errorMessage', 'tableHeaders', 'tableBody', 'refreshBtn', 'retryBtn', 'searchInput', 'prevPage', 'nextPage', 'pageNumbers', 'paginationInfo', 'lastUpdated', 'clearFilters', 'areaFilter', 'responsavelFilter', 'atualizador1Filter', 'connectionStatus', 'viewToggleList', 'viewToggleCard', 'tableViewContainer', 'cardViewContainer'];
    ...
    setView(view) {
        ...
        if (view === 'list') {
            ...
            listBtn.classList.remove('text-slate-700');
            cardBtn.classList.remove('bg-blue-600', 'text-white');
            cardBtn.classList.add('text-slate-700');
        } else {
            cardBtn.classList.add('bg-blue-600', 'text-white');
            cardBtn.classList.remove('text-slate-700');
            listBtn.classList.remove('bg-blue-600', 'text-white');
            listBtn.classList.add('text-slate-700');
        }
        ...
        if (pageData.length === 0) {
            ...
            pageData.forEach(row => {
                ...
                card.innerHTML = `
                    ...
                    ${row.resumo_sim_nao?.toUpperCase() === 'SIM' ? `<span class="mb-auto px-3 py-1 text-xs font-bold text-white bg-indigo-500 rounded-full self-start">TAREFA DE RESUMO</span>` : '<div class="mb-auto"></div>'}
